...
var imgTarget = new Image();   // Create new img element
imgTarget.src = 'target.png'; // Set source path
...
function startGame() 
{
    ...
    target = {
        ...
        xPos: 400,
        yPos: gameScreen.height - cart.y - cart.height / 2 - cart.pendulum.l*graphicScale //dummy values
    };
...
function draw()
{
    ...
    //draw stuff
    gameScreen.ctx.drawImage(imgTarget, target.xPos, target.yPos);
    ...
    cart.pendulum.tipX = cart.x*graphicScale + cart.centerX + x;
    cart.pendulum.tipY = cartYtop + cart.height / 2 - y;
    ...
    gameScreen.ctx.lineTo(cart.pendulum.tipX, cart.pendulum.tipY);
    ...
    if (gameOn)
    {
        ...
        {
            modifyGameScore();
            let graphicalScore = 'Score: ' + gameScore.toString();
            gameScreen.ctx.strokeStyle = 'black'
            gameScreen.ctx.font = '48px serif';
            gameScreen.ctx.strokeText(graphicalScore, 10,50);
        }
        else if (!alive)
        {
            let endScore = 'Final score: ' + gameScore.toString();
            gameScreen.ctx.strokeStyle = 'green'
            gameScreen.ctx.font = '48px serif';
            gameScreen.ctx.strokeText(endScore, 800,200);
        }
    }  

}

function modifyGameScore()
{
    if (cart.pendulum.tipX > target.xPos)
    {
        if (cart.pendulum.tipX < target.xPos + target.width)
        {
            if (cart.pendulum.tipY > target.yPos)
            {
                if (cart.pendulum.tipY < target.yPos + target.height)
                {
                    gameScore += 1;
                }
            }
    ...
    //Check if pendulum angle is to large -> game over
...
function solver()
{
	...
	//PID controller (x)
    let setpoint_error_limit = 1;
    let setpoint_x_carrot = x3p-setpoint_x;
    if (setpoint_x_carrot > setpoint_error_limit) setpoint_x_carrot = setpoint_error_limit;
    if (setpoint_x_carrot < -setpoint_error_limit) setpoint_x_carrot = -setpoint_error_limit;
	let Kp_x = 50;
	let Kd_x = 50;
	let Ki_x = 0;
	let F_x = Kp_x * setpoint_x_carrot + Kd_x * x4p + Ki_x * integral_x;
...
function playGame()
{
    ...
    cart.xDot = 0.0;
    cart.xDotDot = 0.0;
    cart.pendulum.th = 0.0;
    cart.pendulum.thDot = 0.0;
    cart.pendulum.thDotDot = 0.0;


}

function letControllerPlay()
{
    setpoint_x = -1.22;
    controllerOn = true;
    gameOn = true;
